...
## V9-1 — 視覺資產盤點與 UI Polish（已完成）

### 設計概覽

盤點區域、怪物、NPC、角色 UI 所需圖像資產，補強冒險頁與基地頁視覺層次，並整理生成素材規格。
...
|---|---|---|
| T01 | Floor detail 簡化 | ✅ |
| T02 | Battle screen rework | ✅ |
| T03 | Image asset inventory | ✅ |
| T04 | Base image inventory | ✅ |
| T05 | Adventure UI polish | ✅ |
| T05-1 | Elite name / lore fix | ✅ |
| T06 | Monster images | ✅ |
| T06-1 | Resource cleanup | ✅ |
| T07 | Character UI icons | ✅ |

### 目前狀態

V9-1 已完成；相關細節見 `tickets/V9-1/` 與 `art-assets/`。

---

## V9-2 — 主要頁面 Layout Polish（已完成）

### 設計概覽

整理基地 NPC、裝備、背包、採集者、冒險區域卡與樓層 row 的排版密度，讓後期內容量增加後仍可掃讀。
...
| T01 | Base NPC grid | ✅ |
| T02 | Equipment grid | ✅ |
| T03 | Backpack equipment grid | ✅ |
| T04 | Gatherer detail visual | ✅ |
| T05 | Adventure region cards | ✅ |
| T06 | Floor row spacing | ✅ |
| T07 | Region card spacing | ✅ |

### 目前狀態

V9-2 已完成；版本總覽見 `tickets/V9-2/README.md`。

---

## V10-1 — 新手敘事體驗 + 命名系統（已完成）

### 設計概覽

新增開場敘事、英雄命名、職業背景、NPC 首次對話與 NPC 命名，讓玩家第一次進入基地時能理解世界觀與 NPC 關係。

### 完成內容

- `NewPlayerFlowView` 串接開場敘事 → 英雄命名 → 職業選擇。
- `PlayerStateModel` 新增 `hasSeenIntro`、`heroName`、`seenNpcIntroKeysRaw`、`npcNamesRaw` 等欄位。
- `NpcIntroDef` / `NpcIntroSection` 提供 NPC 首次對話與改名入口。
- 職業初始裝備改為配合教程鑄造發放，避免職業選擇後立即出現重複裝備。
- 新增鍛造學徒、裁縫師、飾品師等裝備槽對應 NPC。

### 目前狀態

V10-1 T01–T14 已完成；目前 `ticket-05-starter-equipment.md` 有本地修訂，記錄裝備發放時機改由教程完成。
...
調整教程任務速度、提示 banner、職業戰力預覽、NPC 命名 refactor、教程菁英戰與裁縫師取代皮甲師流程。

### 完成內容

- 教程任務縮短至 2 秒級別，方便第一次體驗快速形成 loop。
- 教程菁英戰改為有傷害但保證可勝，並沿用 BattleLogSheet 顯示戰鬥記錄。
- 生產 Tab NPC 順序重排為主手 → 副手 → 防具 → 飾品 → 廚師 → 製藥師。
- 教程防具流程由裁縫師負責，皮甲師卡片移除。
...
## V10-3 — 引導 UX 審查與原生 UI 整合（已完成）

### 設計概覽

移除重複提示與引導專屬捷徑按鈕，讓玩家透過正常遊戲 UI 完成 step 0 / 2 / 6 / 7。

### 票據摘要

| Ticket | 說明 | 狀態 |
|---|---|---|
| T01 | Tutorial UX audit | ✅ |
| T02 | 移除 step 4 重複橫幅 | ✅ |
| T03 | Step 5 navigation button 決策 | ✅ 採 onAppear 自動推進方向 |
| T04 | Steps 0 / 2 / 6 / 7 引導按鈕整合進原生 UI | ✅ |

### 目前狀態

- `GathererDetailSheet` step 0：保留氣泡，點地點 row 觸發教程採集。
- `CraftSheet` step 2：保留氣泡，基礎武器 row 標示「推薦」，點武器配方觸發教程鑄造。
- `AdventureView` step 5 → 6：進入冒險頁時自動推進；step 6 以樓層 row 高亮觸發教程探索。
- `TailorSheet` step 7：保留氣泡，`wildland_armor` row 標示「推薦」，點配方觸發教程防具任務。

### 驗證狀態

- 主 app target build 通過。
- XCTest target 目前有舊 API 測試需更新：`PlayerStateModel` init 已移除 `hasUsedFirstCraftBoost` / `hasUsedFirstDungeonBoost`，測試仍引用舊參數。
