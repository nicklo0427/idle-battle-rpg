...
struct ArmorSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(player?.npcDisplayName(for: AppConstants.Actor.armorer) ?? "皮甲師")
    ...
    private func startCraft(recipe: CraftRecipeDef) {
        do {
            try TaskCreationService(context: context).createArmorCraftTask(recipeKey: recipe.key)
            dismiss()
        } catch {
