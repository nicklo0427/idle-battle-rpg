...
struct OnboardingService {
    ...
    static func stepInfo(step: Int) -> OnboardingStepInfo? {
        ...
        case 0:
            return .init(hint: [
                .plain("前往"), .action("採集者"), .plain("（樵夫）採集"),
                .material("木材"), .plain("，準備打造"), .equipment("初始武器"),
            ], flavor: "要塞破舊，需要新的裝備來重整。")
        case 1:
            return .init(hint: [
                .plain("等待"), .action("採集"), .plain("完成…"),
            ], flavor: "斧頭聲迴盪在林間，木材一根根累積。")
        case 2:
            return .init(hint: [
                .plain("前往"), .action("鑄造師"), .plain("打造你的"),
                .equipment("初始武器"),
            ], flavor: "原料到手，是時候打出第一把武器了。")
        case 3:
            return .init(hint: [
                .plain("前往「"), .action("角色"), .plain("」頁裝備你的"),
                .equipment("初始武器"),
            ], flavor: "鐵鎚聲落定，武器剛淬煉完工，該親手把它裝上了。")
        case 4:
            return .init(hint: [
                .plain("前往"), .action("冒險"), .plain("頁，挑戰"),
                .location("金穗之野"), .plain("的"), .action("菁英敵人"), .plain("！"),
            ], flavor: "手握武器，準備第一場真正的戰鬥。")
        case 5:
            return .init(hint: [
                .plain("前往「"), .action("生產"), .plain("」→「"),
                .action("裁縫師"), .plain("」製作你的第一件"), .equipment("防具"),
            ], flavor: "有了武器，接下來要學會如何護住自己。")
        case 6:
            return .init(hint: [
                .plain("前往「"), .action("冒險"), .plain("」→ "),
                .location("金穗之野"), .plain("，探索獲取"), .material("防具素材"),
            ], flavor: "荒野中的獸皮正是打造護甲的材料。")
        case 7:
            return .init(hint: [
                .material("素材"), .plain("已備妥！前往"), .action("裁縫師"),
                .plain("完成"), .equipment("防具"), .plain("鑄造"),
            ], flavor: "皮革的氣味充滿小屋，護甲即將成形。")
        case 8:
            return .init(hint: [
                .plain("前往「"), .action("角色"), .plain("」→「"),
                .equipment("裝備欄"), .plain("」，親手穿上"), .equipment("防具"),
            ], flavor: "護甲已放進背包，真正穿上才算準備好。")
        case 9:
            return .init(hint: [
                .plain("前往"), .action("冒險"), .plain("，在"),
                .location("金穗之野第 1 層"), .plain("開始第一次正式"), .action("出征"),
            ], flavor: "裝備齊了，該讓英雄走一趟真正的路。")
        case 10:
            return .init(hint: [
                .plain("觀看"), .action("戰鬥"), .plain("並"), .action("收下獎勵"),
                .plain("，解鎖"), .action("升級與技能"),
            ], flavor: "戰鬥經驗會把英雄推向第一個成長節點。")
        case 11:
            return .init(hint: [
                .plain("在「"), .action("角色"), .plain("」頁分配"),
                .action("屬性點"), .plain("並"), .action("確認加點"),
            ], flavor: "力量、護甲、速度，先選一個你想強化的方向。")
        case 12:
            return .init(hint: [
                .plain("到「"), .action("技能"), .plain("」頁，"),
                .action("升階"), .plain("或"), .action("配備"), .plain("第一個主動技能"),
        ...
        case 14:
            return .init(hint: [
                .plain("回到"), .equipment("裝備欄"), .plain("，把"),
                .equipment("武器"), .plain("強化到"), .equipment("+1"),
            ], flavor: "一點強化，就能讓早期戰鬥舒服很多。")
        case 15:
            return .init(hint: [
                .plain("前往"), .action("商人"), .plain("，購買"),
                .material("小麥種子"),
            ], flavor: "農田需要種子，市集是最穩定的補給點。")
        case 16:
            return .init(hint: [
                .plain("找"), .action("農夫"), .plain("在"),
                .action("農田 1"), .plain("種下"), .material("小麥種子"),
            ], flavor: "農田會把種子變成料理與藥水需要的作物。")
        case 17:
            return .init(hint: [
                .plain("找"), .action("廚師"), .plain("製作第一份"),
                .equipment("魚肉燉鍋"),
            ], flavor: "料理可在出征前攜帶，提供一段時間的屬性加成。")
        case 18:
            return .init(hint: [
                .plain("找"), .action("製藥師"), .plain("製作第一瓶"),
                .equipment("小型藥水"),
            ], flavor: "藥水會在戰鬥危急時自動救場。")
        case 19:
            return .init(hint: [
                .plain("回到"), .action("冒險"), .plain("，攜帶"),
                .equipment("料理"), .plain("與"), .equipment("藥水"), .plain("再出征一次"),
            ], flavor: "把補給帶上戰場，完整循環就接起來了。")
        case 20:
            return .init(hint: [
                .plain("找"), .action("伐木工阿森"), .plain("開啟"),
                .action("狀態及養成"), .plain("，升到"), .action("T1"),
            ], flavor: "NPC 也能養成，升級後會獲得技能點。")
        case 21:
            return .init(hint: [
                .plain("在伐木工"), .action("技能"), .plain("頁投入"),
                .action("砍伐熟練"),
            ], flavor: "技能點會讓採集產出更穩，之後也能套用到其他 NPC。")
        case 22:
            return .init(hint: [
                .plain("前往「"), .action("角色"), .plain("」→「"),
                .action("成就"), .plain("」查看已解鎖成就"),
